...
const SearchListings = () => {
  ...
  const handleChange = (e) => {
    ...
    navigate(`/display-all?${urlParams.toString()}`);
  };
  ...
  useEffect(() => {
    ...
    async function searchListingsBasedOnQuery() {
      ...
      try {
        const res = await axios.get(`${import.meta.env.VITE_BASE_URL}api/properties/listings/display-all?${searchQuery}`);
        // const res = await axios.get(`http://localhost:5000/api/properties/listings/display-all?${searchQuery}`);
  ...
  const showMoreListings = async () => {
    ...
    try {
      const res = await axios.get(`${import.meta.env.VITE_BASE_URL}api/properties/listings/fetch?fetch?${searchQuery}`);
      // const res = await axios.get(`http://localhost:5000/api/properties/listings/display-all?${searchQuery}`);
    ...
    <div className='flex flex-col md:flex-row'>
      <div className='p-7  border-b-2 md:border-r-2 md:min-h-screen'>
        <form onSubmit={handleFormSubmit} className='flex flex-col gap-8'>
          <div className='flex items-center gap-2'>
            ...
            <input
              type='text' id='search' placeholder='Search...' className='border rounded-lg p-3 w-full'
              value={sideBarData?.search} onChange={handleChange} />
          </div>
          <div className='flex gap-2 flex-wrap items-center'>
            <label className='font-semibold'>Type:</label>
            <div className='flex gap-2'>
              <input
                type='checkbox' id='all' className='w-5' onChange={handleChange} checked={sideBarData?.type === 'all'} />
              <span>Rent & Sale</span>
            </div>
            <div className='flex gap-2'>
              <input
            ...
            <div className='flex gap-2'>
              <input
                type='checkbox'
                id='offer' className='w-5' onChange={handleChange} checked={sideBarData?.offer} />
              <span>Offer</span>
            </div>
          </div>
          <div className='flex gap-2 flex-wrap items-center'>
            ...
            <div className='flex gap-2'>
              <input
                type='checkbox' id='furnished' className='w-5' onChange={handleChange} checked={sideBarData?.furnished} />
              <span>Furnished</span>
            </div>
          </div>
          <div className='flex items-center gap-2'>
      ...
      <div className='p-7  border-b-2 '>
        ...
        <div className='flex flex-wrap gap-4'>
          {
            !loading && listings && listings?.map((listing: ListingType) => {
